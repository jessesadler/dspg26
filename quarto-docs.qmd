...
Quarto documents provide a way to mix together [Markdown text](markdown-syntax.qmd) with code and its results. Whereas **R scripts** are designed primarily for writing and running code---with comments (`#`) giving a means to document the code---**Quarto documents** allow you to write prose alongside code. You can then output the Quarto document into a number of formats, including web pages, PDFs, slides, and even Word documents. Projects of connected Quarto documents can create [dashboards](https://quarto.org/docs/dashboards/), [books](https://quarto.org/docs/books/), [websites](https://quarto.org/docs/websites/).[^1] Why might you use Quarto documents instead of an R script?
...
To get familiar with Quarto, **please download** this [Quarto template document](files/quarto-intro.qmd), place it in your project, read through it, and then render it into an HTML document.
...
2. Markdown text.
3. Code chunks set off by three back ticks and the coding language in curly brackets (```` ```{r} ````).

![A Quarto document in RStudio.](files/img/quarto-doc.png){#fig-quarto-doc width=90% fig-alt="A screen shot of a Quarto document in RStudio showing the three main components of a YAML header, markdown content, and code chunks."}
...
The available keys can be found in the documentation for each type of output.[^2] Many options are shared across document output types, but some are not. See the [HTML options](https://quarto.org/docs/reference/formats/html.html) for the widest number of options. It can be difficult to write these YAML headers. There are a lot of different options, and the indentation level must be formatted correctly. Happily, in practice, it is unlikely to be as difficult it looks. Firstly, you will not need to write complex YAML headers often. Secondly, you can figure out settings that work for you and then copy and paste them into other documents. There are also ways for YAML options to be used for all documents in a [project](https://quarto.org/docs/projects/quarto-projects.html) such as a website.
...
### 3. Code chunks
With Markdown as the default editing environment, your code must be placed in code chunks set off by three back ticks followed by the coding language in curly brackets (```` ```{r} ````). They look like this:
...
2. Typing the chunk delimiters: ```` ```{r} ```` to start the chunk and ```` ``` ```` to end the chunk.
...
[^1]: This website is a Quarto book.

[^2]: See the [Output formats](#output-formats) section for more details on output formats.
